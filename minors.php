...
for($j = 0; $j < count($files_to);$j++){
  ...
  if(substr($_GET['way'],-2) == '17') $indexSheet = 2;// у 2 курса
  else if(substr($_GET['way'],-2) == '16') $indexSheet = 4;// у 3 курса
    ...
    if(is_numeric($rest_new)){
      ...
      if($poi != ''){
        ...
        if($_GET['group_id'] == '1'){
          ...
          if($g1 == $g2) $group = 'Майнор | Лекция';//если объединена
          else $group = 'Майнор | Семинар';//если НЕ объединена
...
function parse_to($dayR,$way_learn,$group_id, $week_one, $week_two, $week_three, $week_five, $week_six, $wp1, $wp2, $wp3, $wp4, $wp5, $wp6, $we_pa_num1, $we_pa_num2, $we_pa_num3, $we_pa_num4, $we_pa_num5, $we_pa_num6){
  ...
  //strtotime("+1 day");
  $objPHPExcel = PHPExcel_IOFactory::load("files/".searchWeek($dayR, $week_one, $week_two, $week_three, $week_five, $week_six, $wp1, $wp2, $wp3, $wp4, $wp5, $wp6));//выбираем нужный excel файл и дату(на завтра) date("d.m.Y", $date_tom))
  if(substr($way_learn,-2) == '17') $indexSheet = 2;// у 2 курса
  else if(substr($way_learn,-2) == '16') $indexSheet = 4;// у 3 курса
  $objPHPExcel->setActiveSheetIndex($indexSheet);//выбрали первый лист
  $aSheet = $objPHPExcel->getActiveSheet();//перешли на него
...
function parse_more($dayR,$way_learn,$group_id, $week_one, $week_two, $week_three, $week_five, $week_six, $wp1, $wp2, $wp3, $wp4, $wp5, $wp6, $we_pa_num1, $we_pa_num2, $we_pa_num3, $we_pa_num4, $we_pa_num5, $we_pa_num6){
  ...
  for($i = 4; $i <= 46; $i++){
    ...
    if($rty == 0){
      ...
      $ot = get_prev_day($rty, $dayR, $we_pa_num1, $we_pa_num2, $we_pa_num3, $we_pa_num4, $we_pa_num5, $we_pa_num6);//3
      $pol = $aSheet->getCell("B".($ot+1));
      if(substr($pol,0,1) == 2) $olop = '$';
      if(substr($pol,0,1) == 3) $olop2 = '$';
      $do = $pairs + $ot + 1;
...
$molv = 2;
if(substr($_GET['way'],-2) == '17') $molv = 2;// у 2 курса
else if(substr($_GET['way'],-2) == '16') $molv = 4;// у 3 курса

if(isset($_GET['update']) or isset($_POST['update'])) update();
else if(isset($_GET['gnm']) or isset($_POST['gnm'])){
  $nul = revArr($molv, $_GET['day'], $week_one, $week_two, $week_three, $week_five, $week_six, $wp1, $wp2, $wp3, $wp4, $wp5, $wp6);
  for($i = 0; $i < count($nul); $i++) echo $nul[$i].'<br>';

}else if(isset($_GET['tr']) or isset($_POST['tr'])){
  parse_more($_GET['day'], $_GET['way'], $_GET['group_id'], $week_one, $week_two, $week_three, $week_five, $week_six, $wp1, $wp2, $wp3, $wp4, $wp5, $wp6, $we_pa_num1, $we_pa_num2, $we_pa_num3, $we_pa_num4, $we_pa_num5, $we_pa_num6);
